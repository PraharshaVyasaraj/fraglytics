...
import { generateMasterExcel, generateRawCSV, generateClipboardBridge, generateJSON, generateAuditCSV, generateStandingsCSV } from '../services/exportEngine';
import { Database, FileSpreadsheet, FileText, Clipboard, X, CheckCircle2, Download, Table, Settings2, ShieldCheck, FileJson, ScrollText, ListOrdered } from 'lucide-react';
                ...
                <div className="w-full md:w-1/3 bg-tactical-dark border-r border-tactical-gray p-6 flex flex-col">
                     ...
                     <div className="space-y-6 flex-1">
                        ...
                        <div className="space-y-3">
                             <label className="text-[10px] font-mono uppercase text-tactical-light font-bold flex items-center gap-2"><Settings2 className="w-3 h-3"/> Field Selectors</label>
                             ...
                             <div className="flex items-center justify-between p-2 bg-black/40 rounded-sm border border-white/5">
                                 <span className="text-xs text-white">Advanced Metrics (Agg/Eff)</span>
                                 <input type="checkbox" checked={config.includeAdvanced} onChange={e => setConfig({...config, includeAdvanced: e.target.checked})} className="accent-tactical-red"/>
                             </div>
                             <div className="flex items-center justify-between p-2 bg-black/40 rounded-sm border border-white/5">
                ...
                <div className="flex-1 p-8 bg-[#0E0E0E] relative overflow-y-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                        ...
                        <button onClick={handleExcel} className="col-span-1 md:col-span-2 group flex items-center gap-6 p-6 border border-tactical-gray hover:border-green-500 hover:bg-green-500/5 transition-all rounded-sm text-left">
                         ...
                         {/* CARD 5: BRIDGE */}
                         <button onClick={handleClipboard} className="col-span-1 md:col-span-2 group flex items-center gap-6 p-6 border border-tactical-gray hover:border-yellow-500 hover:bg-yellow-500/5 transition-all rounded-sm text-left">
                            <div className="p-4 bg-tactical-dark border border-tactical-gray rounded-sm group-hover:border-yellow-500 group-hover:text-yellow-500 transition-colors">
                                <Clipboard className="w-8 h-8 text-white group-hover:text-yellow-500" />
                            </div>
                            <div>
                                <h3 className="text-xl font-bold text-white uppercase group-hover:text-yellow-500 transition-colors">Spreadsheet Bridge</h3>
                                <p className="text-xs text-tactical-light font-mono mt-1">Smart-Copy for Google Sheets (Tab Delimited)</p>
                            </div>
                            <div className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity">
